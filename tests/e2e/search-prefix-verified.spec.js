...
test.describe('поиск: prefix-верифицированный результат содержит искомую фразу', () => {
  ...
  test('encodePhraseToCoords: prefix-верифицированный результат для «мир» найден', async ({ page }) => {
    await page.goto('/');

    /* Проверяем через JS API: encodePhraseToCoords должен содержать фразу */
