...
test.describe('Кнопка «Открыть» после поиска — навигация и текст', () => {
  ...
  test('messenger: после клика «Открыть» в random-режиме — на странице есть искомая фраза', async ({ page }) => {
    await page.addInitScript(() => {
      localStorage.setItem('babelTheme', 'messenger');
      localStorage.setItem('babelLibraryMode', 'random');
    });
    ...
    /* В random-режиме байтовые адреса декодируются корректно и фраза видна */
  ...
  test('messenger: после клика «Открыть» в human-режиме — страница загружается без ошибки', async ({ page }) => {
    await page.addInitScript(() => {
      localStorage.setItem('babelTheme', 'messenger');
    ...
    /* Ждём рендера контента — в human-режиме страница показывается через prefix codec */
    await page.waitForFunction(() => {
      return document.querySelectorAll('.msg-bubble-page .msg-text').length > 0;
    }, { timeout: 30000 });

    /* Страница должна загрузиться (текст может отличаться от искомой фразы
       т.к. адрес создан для byte-level декодирования, а в human-режиме
       используется prefix codec — это ожидаемое поведение) */
    const pageLoaded = await page.evaluate(() => {
      const textEls = document.querySelectorAll('.msg-bubble-page .msg-text');
      if (!textEls || textEls.length === 0) return { error: 'no text elements' };
      const text = Array.from(textEls).map(el => el.textContent).join('');
      return { loaded: true, textLength: text.length, snippet: text.slice(0, 200) };
    });

    expect(pageLoaded.error).toBeUndefined();
    expect(pageLoaded.loaded).toBe(true);
  });
...
test.describe('Worker: координаты в результатах поиска содержат x, y, z', () => {
  ...
  test('searchMultiMode через Worker возвращает x, y, z в coordinates', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      /* Проверяем любой доступный режим (prefix удалён из searchMultiMode) */
      const firstMode = Object.keys(modes)[0];
      if (!firstMode) return { error: 'no search results' };
      const firstResult = modes[firstMode];

      return {
        mode: firstMode,
        hasX: 'x' in firstResult.coordinates,
        hasY: 'y' in firstResult.coordinates,
        hasZ: 'z' in firstResult.coordinates,
        xIsNonZero: Number(firstResult.coordinates.x) !== 0,
        yIsNonZero: Number(firstResult.coordinates.y) !== 0,
        coords: {
          x: String(firstResult.coordinates.x),
          y: String(firstResult.coordinates.y),
          z: String(firstResult.coordinates.z),
        },
      };
    });
...
test.describe('Режим библиотеки — поиск совместим со всеми режимами', () => {

  test('результаты поиска НЕ содержат engine= в URL (совместимы с обоими режимами)', async ({ page }) => {
    await page.addInitScript(() => {
      localStorage.setItem('babelTheme', 'messenger');
      localStorage.setItem('babelLibraryMode', 'human');
    });
    ...
    /* Ни одна ссылка не должна содержать engine= — режим определяется текущей настройкой */
    const count = await openLinks.count();
    for (let i = 0; i < count; i++) {
      const href = await openLinks.nth(i).getAttribute('href');
      expect(href).not.toContain('engine=');
    }
  });
