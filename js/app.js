(() => {
  ...
  function renderAbout() {
    ...
    <section class="about fade-in">
      ...
      <p>Координата Z определяет «температуру» страницы — её удалённость от человеческого текста. Z = 1 даёт температуру ≈ 0: частые токены, строгая грамматика, читаемый текст. Z = 10<sup>6</sup> даёт температуру ≈ 0.6: смешанный текст с редкими токенами. Z = 10<sup>10</sup> и выше — чистый шум, хаос, сырые символы. Это «гравитация языка»: человеческий текст притягивается к началу координат, а шум отталкивается на периферию.</p>

      <h2>Детерминированность</h2>
      <p>Каждый адрес (X, Y, Z) всегда производит одну и ту же страницу. Адрес используется в двух ролях: hash(X, Y, Z) определяет разнообразие страницы (какие именно токены выбраны), а magnitude(Z) определяет уровень шума (насколько редкие токены допустимы). Выбор полностью детерминированный — адрес является источником псевдослучайных выборов, а не запускает случайность.</p>
      ...
      <p>Алфавит библиотеки: 256 символов = 1 байт. Русские буквы (33), английские (26), цифры, знаки препинания, эмодзи. Длина страницы — 4096 символов.</p>
      ...
      <h2>Аффинная перестановка (legacy)</h2>
      <p>Для обратной совместимости со старыми base64-адресами сохранена аффинная перестановка над Z/(2<sup>32768</sup>):</p>
